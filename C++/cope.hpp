...
#define _printVscodeLink(file, line, func)                                     \
  printsl(color((char *)"\n\t", LINK_COLOR));                                  \
...
#endif
#endif
// printsl(color(fmt(__STRING(File __STRING(__FILE__)[,] line __LINE__[,] in
// __func__)), LINK_COLOR))

// #define leave(exitStatus) { debugs(__STRING(Exiting with code exitStatus));
// exit(exitStatus); }

// #define assert(test) printDebugCount(); printMetaData(); assert(test);

#define printMetaData()                                                        \
  _printMetaData(__LINE__, (char *)__FUNC__, (char *)__FILE__)
...
#define debug_1(var)                                                           \
  {                                                                            \
    printDebugCount();                                                         \
    printMetaData();                                                           \
    printfclrsl(DEBUG_COLOR, "%s %s = ", _typename(var), #var);                \
    printsl(var);                                                              \
    nl();                                                                      \
  }
#define debug_2(var, clr)                                                      \
  {                                                                            \
    printDebugCount();                                                         \
    printMetaData();                                                           \
    printfclrsl(fmt("\x1b[3%dm", clr).c_str(), "%s %s = ", _typename(var),     \
                #var);                                                         \
    printsl(var);                                                              \
    nl();                                                                      \
  }
#define debugs(string)                                                         \
  {                                                                            \
    printDebugCount();                                                         \
    printMetaData();                                                           \
    printclr(REGULAR_DEBUG_COLOR, string);                                     \
    printVscodeLink();                                                         \
  }
#define debugf(string, ...)                                                    \
  {                                                                            \
    printDebugCount();                                                         \
    printMetaData();                                                           \
    printfclr(REGULAR_DEBUG_COLOR, string, __VA_ARGS__);                       \
    printVscodeLink();                                                         \
  }
#define shout                                                                  \
  {                                                                            \
    printDebugCount();                                                         \
    printMetaData();                                                           \
    printclrsl(REGULAR_DEBUG_COLOR, (char *)"HERE! HERE!", 1);                 \
    printVscodeLink();                                                         \
  }
...
// #define debugged(type, var) __debugged<type>(var, (char*)#var,
// _typename(var))

#define debug_X(x, var, color, FUNC, ...) FUNC
#define _debug(...)                                                            \
  debug_X(, ##__VA_ARGS__, debug_2(__VA_ARGS__), debug_1(__VA_ARGS__),         \
          debug_0(__VA_ARGS__))
//
...
#define todo(feature)                                                          \
  {                                                                            \
    printDebugCount();                                                         \
    printMetaData();                                                           \
    printclrsl(TODO_COLOR, "Todo: ");                                          \
    printclr(REGULAR_DEBUG_COLOR, feature);                                    \
    printVscodeLink();                                                         \
  }

#define uchar unsigned char

// This is to ternary as bool is to binary
union tern {
  char value;
};
const tern True{1};
...
static void leave(int status) { leave(status, ""); }
...
static std::string cStr(std::string str) { return str + '\0'; }

static std::string cStr(char *str) { return std::string(str) + '\0'; }
...
inline std::vector<char> getCmdOutV(const std::string command, bool verbose,
  ...
  pipe = popen(cmd, "r");
  if (pipe == NULL) {
    _debug("Error: Failed to open pipe", 5);
  }
  ...
  int rtn = pclose(pipe);
  if (rtn and exitOnFail)
    leave(rtn, fmt("\"%s\" returned error: %d", cmd, rtn));
...
macro #define XXX_X(x,A,B,C,D,FUNC, ...)  FUNC
...
#endif /* DEBUG */
#ifdef MAIN_PROGRAM
const char jlss_id_debug_h[] =
    "@(#)$Id: debug.h,v 3.6 2008/02/11 06:46:37 jleffler Exp $";
#endif /* MAIN_PROGRAM */
#endif /* lint */
